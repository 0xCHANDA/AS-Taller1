# Bitácora de decisiones tomadas con apoyo de IA
...
| Propuesta | Decisión | Qué pensamos como equipo |
|---|---|---|
...
| Usar una interfaz de inventario con las operaciones necesarias para agregar, retirar y consultar productos. | **Aceptada con ajustes** | La idea era correcta, pero usamos `IInventarioVendible<T>` para mantener el tipo concreto de cada inventario. Así `Potrero`, `InventarioLacteos` e `InventarioPieles` comparten el mismo contrato sin perder seguridad de tipos. |
| Volver inmutable `Producto.Nombre` para asegurar que se venda el mismo objeto que se retira. | **Ajustada** | Quitar el setter cambiaba la API que ya existía. La solución fue registrar en la venta exactamente el objeto devuelto por `retirar`. |
